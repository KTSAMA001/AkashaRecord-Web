...
status: ✅ 已验证
description: >-
  在 Unity 2022.3.x（Meta Quest / Android / Vulkan）平台，开启
  `PlayerSettings.strictShaderVariantMatching` 后，Shader Graph 中使用
  `multi_compile_local` 的 Enum 关键字（如 `_DISPLAYMODE_NORMAL _DISPLAYMODE_SP1`）会持续报
  `variant not found` 错误。画面显示正常，但控制台不断刷错。关闭严格匹配后错误消失。 本记录已按官方帖子与 Issue Tracker
  对齐到版本维度：**“strict 下报 variant not found”是一个问题族，不同版本修复的是不同子问题。你在 2022.3.39f1
  遇到的这类，并不被已修复的 Deferred 子问题覆盖。**
